...
class StartController extends Controller
{
    ...
    public function getCoursesByFilter(Request $request): JsonResponse
    {
        ...
        // Si hay término de búsqueda, unimos para buscar por título, categoría, carrera y dueño
        if ($term !== '') {
    $like = $this->like($term);

    // Subconsulta SIN GROUP BY usando EXISTS (compatible con ONLY_FULL_GROUP_BY)
    $relevanceSub = DB::table('courses as c')
        ...
        ->having('relevance', '>', 0)      // permitido usar alias en HAVING
        ->orderByDesc('relevance');

    // Une la subconsulta a la query principal y ordena por relevancia
    $query
        ->joinSub($relevanceSub, 'sr', 'sr.course_id', '=', 'courses.id')
        ->addSelect(DB::raw('sr.relevance'))
        ->orderByDesc('sr.relevance');
} else {
            // Sin término: mantenemos tus órdenes por filtro
        ...
        // fallback de orden para empates en búsqueda
        $query->orderByDesc('total_stars')
              ->orderByDesc(DB::raw('registrations_count + saved_courses_count'))
              ->orderByDesc('created_at');
        ...
        if ($hasMore) $courses = $courses->slice(0, $perPage);
